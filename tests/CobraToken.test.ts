...
describe('CobraToken', function () {
  ...
  describe('#breed()', async () => {
    ...
    it('should revert when the amount paid is too little', async () => {
      await expect(
        cobraToken.connect(userAccount1).breed(matronId, sireId, {
    ...
    it('should pass the correct request data to the Oracle', async () => {
      await cobraToken.connect(userAccount1).breed(matronId, sireId, {
      ...
      await new Promise<void>((resolve) => {
        oracle.once(
          ...
          async (
            ...
            expect(max).equal(999999999)
            expect(owner).equal(userAccount1.address)
            expect(matronId).equal(200)
            expect(sireId).equal(400)
    ...
    it('should emit a Success event when a Cobra is bred', async () => {
      await expect(
        cobraToken.connect(userAccount1).breed(matronId, sireId, {
          value: ethers.utils.parseEther('0.05')
        })
